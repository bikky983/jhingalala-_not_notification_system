document.addEventListener('DOMContentLoaded', function() {
    ...
    function createTreemap(dataArray) {
        ...
        // Create the treemap layout with smaller cells
        const treemap = d3.treemap()
            .size([width, height])
            .paddingTop(25) // Slightly reduced from 30
            .paddingBottom(2) // Reduced from 10
            .paddingRight(2) // Reduced from 5
            .paddingLeft(2) // Reduced from 5
            .paddingInner(2) // Reduced from 4 to make cells smaller
            .round(true);
        ...
        const root = d3.hierarchy(treeData)
            .sum(d => {
                ...
                // This makes the size difference between large and small stocks less extreme
                if (d.value) {
                    // Apply logarithmic scaling to volume to reduce extremes
                    return Math.log(d.value + 1) * 100;
                }
        ...
        // Add stock symbol labels - more compact
        cell.append('text')
            .attr('class', 'treemap-cell-label')
            .attr('x', 3) // Reduced from 6 to 3
            .attr('y', 12) // Reduced from 18 to 12
            .text(d => d.data.name)
            .attr('font-size', '10px') // Reduced from 13px to 10px
            .attr('font-weight', 'bold');
        
        // Add percent change labels - more compact
        cell.append('text')
            .attr('class', 'treemap-value-label')
            .attr('x', 3) // Reduced from 6 to 3
            .attr('y', 22) // Reduced from 36 to 22
            .text(d => `${d.data.originalValue.toFixed(1)}%`)
            .attr('font-size', '9px') // Reduced from 12px to 9px
            .attr('fill', d => d.data.originalValue >= 0 ? '#008800' : '#cc0000');
        
        // Add volume info - more compact or remove if cells are too small
        cell.append('text')
            .attr('class', 'treemap-volume-label')
            .attr('x', 3) // Reduced from 6 to 3
            .attr('y', 32) // Reduced from 52 to 32
            .text(d => {
            ...
            .attr('font-size', '8px'); // Reduced from 11px to 8px
