document.addEventListener('DOMContentLoaded', function() {
    ...
    function createTreemap(dataArray) {
        ...
        cell.append('rect')
            ...
            .attr('fill', d => {
                ...
                if (isNearSupportLevel(d.data.name, d.data.close)) {
                    // Blue color for near-support stocks
                    return '#1976D2'; // Blue
                } else {
            ...
            .on('mouseover', function(event, d) {
                ...
                if (isNearSupportLevel(d.data.name, d.data.close)) {
                    tooltipContent += `
                        <div class="tooltip-badge support-badge">Near Support Level</div>
                    `;
                }
    ...
    function isNearSupportLevel(symbol, currentPrice) {
        try {
            const userStocksData = localStorage.getItem('userStocks');
            if (!userStocksData) return false;
            
            const userStocks = JSON.parse(userStocksData);
            if (!Array.isArray(userStocks)) return false;
            
            const stockInfo = userStocks.find(s => s && s.symbol === symbol);
            if (!stockInfo) return false;
            
            // Check proximity to support levels (within 3%)
            ...
            if (supportPrice1 && !isNaN(supportPrice1)) {
                ...
                if (Math.abs(diff) <= threshold) return true;
            }
