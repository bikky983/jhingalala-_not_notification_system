document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadDashboardData() {
        try {
            ...
            if (watchlistData) {
                ...
                console.log(`Loaded ${dashboardStocks.length} stocks from dashboard with their sectors`);
            }
        } catch (error) {
    ...
    function processWeeklyData() {
        ...
        Object.keys(stockGroups).forEach(symbol => {
            ...
            if (records.length > 0) {
                ...
                // If no sector is available from dashboard, use Unclassified
                if (!sector) {
                    sector = 'Unclassified';
                }
    ...
    function createTreemap(dataArray) {
        ...
        dataArray.forEach(stock => {
            // Default to "Unclassified" if no sector is available
