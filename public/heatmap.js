document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadDashboardData() {
        try {
            // Reset the dashboardStocks array
            dashboardStocks = [];
            
            // Try different methods to get dashboard stocks
            
            // Method 1: Get stocks from watchlistData
            ...
            if (watchlistData) {
                try {
                    ...
                    console.log(`Loaded ${dashboardStocks.length} stocks from watchlist data`);
                } catch (e) {
                    console.error('Error parsing watchlist data:', e);
                }
            }
            
            // Method 2: Get stocks from folder data
            ...
            if (foldersData) {
                try {
                                    ...
                                    try {
                                        ...
                                        if (Array.isArray(folderStocks)) {
                                            folderStocks.forEach(stock => {
                                                if (typeof stock === 'string' && !dashboardStocks.includes(stock)) {
                    ...
                    console.log(`After folder scan: Total of ${dashboardStocks.length} dashboard stocks loaded`);
                } catch (e) {
                    console.error('Error parsing folders data:', e);
                }
            }
            
            // Method 3: Direct folder scan if we still don't have stocks
            if (dashboardStocks.length === 0) {
                console.log('No stocks found. Scanning localStorage for folder information directly.');
                ...
                for (let i = 0; i < localStorage.length; i++) {
                    ...
                    if (key && key.startsWith('folder_') && key !== 'folder_all') {
                        try {
                            ...
                            console.error(`Error parsing stocks from ${key}:`, e);
                        }
                    }
                }
                console.log(`After direct folder scan: ${dashboardStocks.length} dashboard stocks loaded`);
            }
        } catch (error) {
    ...
    function createTreemap(dataArray) {
        ...
        // Prepare data in hierarchical structure for treemap without sector segregation
        const treeData = {
            name: "All Stocks",
            children: dataArray.map(stock => ({
                name: stock.symbol,
