document.addEventListener('DOMContentLoaded', function() {
    ...
    const supportThresholdSlider = document.getElementById('supportThreshold');
    ...
    let supportThreshold = 3; // Default threshold percentage
    ...
    function initEventListeners() {
        ...
        if (supportThresholdSlider) {
            supportThresholdSlider.addEventListener('input', function() {
                supportThreshold = parseFloat(this.value);
                if (thresholdValueDisplay) {
                    thresholdValueDisplay.textContent = supportThreshold;
                }
        ...
        cell.append('rect')
            ...
            .attr('fill', d => {
                ...
                if (supportLevel > 0) {
                    ...
                    switch (supportLevel) {
                        case 1: return '#42A5F5'; // Light blue for Support 1
                        case 2: return '#FFA726'; // Light orange for Support 2
                        case 3: return '#8D6E63'; // Light brown for Support 3
            ...
            .on('mouseover', function(event, d) {
                ...
                if (supportPrice1 || supportPrice2 || supportPrice3 || upperLimit) {
                    ...
                    if (supportPrice1) {
                        ...
                        const diff1Class = Math.abs(diff1Value) <= supportThreshold ? 'near-support' : '';
                        tooltipContent += `
                            <div class="tooltip-row ${diff1Class}">
                                <span class="tooltip-label">Proximity:</span>
                                <span>${Math.abs(diff1Value).toFixed(2)}% ${Math.abs(diff1Value) <= supportThreshold ? '(Near!)' : ''}</span>
                            </div>
                        `;
                    }
                    ...
                    if (supportPrice2) {
                        ...
                        const diff2Class = Math.abs(diff2Value) <= supportThreshold ? 'near-support' : '';
                        tooltipContent += `
                            <div class="tooltip-row ${diff2Class}">
                                <span class="tooltip-label">Proximity:</span>
                                <span>${Math.abs(diff2Value).toFixed(2)}% ${Math.abs(diff2Value) <= supportThreshold ? '(Near!)' : ''}</span>
                            </div>
                        `;
                    }
                    ...
                    if (supportPrice3) {
                        ...
                        const diff3Class = Math.abs(diff3Value) <= supportThreshold ? 'near-support' : '';
                        tooltipContent += `
                            <div class="tooltip-row ${diff3Class}">
                                <span class="tooltip-label">Proximity:</span>
                                <span>${Math.abs(diff3Value).toFixed(2)}% ${Math.abs(diff3Value) <= supportThreshold ? '(Near!)' : ''}</span>
                            </div>
                        `;
                    }
    ...
    function isNearSupportLevel(symbol, currentPrice) {
        try {
            ...
            if (supportPrice1 && !isNaN(supportPrice1)) {
                ...
                if (Math.abs(diff) <= threshold) return 1;
            }
            ...
            if (supportPrice2 && !isNaN(supportPrice2)) {
                ...
                if (Math.abs(diff) <= threshold) return 2;
            }
            ...
            if (supportPrice3 && !isNaN(supportPrice3)) {
                ...
                if (Math.abs(diff) <= threshold) return 3;
            }
